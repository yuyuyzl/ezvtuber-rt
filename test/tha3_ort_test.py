"""
Test cases for tha_ort.py - THAORT and THAORTNonDefault classes
Tests different model configurations (fp16/fp32, seperable/standard, with/without eyebrow)
"""
import unittest
import numpy as np
import cv2
...
class TestTHAORTBase(unittest.TestCase):
...
class TestTHAORTSeperableFP16WithEyebrow(TestTHAORTBase):
    """Test THAORT with seperable/fp16 model and eyebrow enabled"""
    ...
    def setUpClass(cls):
        ...
        cls.engine = THA3ORTSessions(cls.MODEL_DIR, use_eyebrow=True)
    
    def test_dtype_detection(self):
        """Test that fp16 is correctly detected from path"""
        self.assertEqual(self.engine.dtype, np.float16)
    
    def test_seperable_detection(self):
        """Test that seperable is correctly detected from path"""
        self.assertTrue(self.engine.seperable)
...
class TestTHAORTSeperableFP16NoEyebrow(TestTHAORTBase):
    """Test THAORT with seperable/fp16 model and eyebrow disabled"""
    ...
    def setUpClass(cls):
        ...
        if not os.path.exists(cls.MODEL_DIR):
            raise unittest.SkipTest(f"Model directory not found: {cls.MODEL_DIR}")
        cls.engine = THA3ORTSessions(cls.MODEL_DIR, use_eyebrow=False)
    ...
    def test_eyebrow_mode_disabled(self):
        ...
        self.assertTrue(hasattr(self.engine, 'combiner_decode_cut'))
...
class TestTHAORTStandardFP16WithEyebrow(TestTHAORTBase):
    """Test THAORT with standard/fp16 model and eyebrow enabled"""
    ...
    def test_dtype_detection(self):
        """Test that fp16 is correctly detected"""
        self.assertEqual(self.engine.dtype, np.float16)
    
    def test_seperable_detection(self):
        """Test that standard (non-seperable) is correctly detected"""
        self.assertFalse(self.engine.seperable)
    
    def test_inference_standard_model(self):
        """Test inference with standard model"""
        self.engine.update_image(self.test_image)
        pose = np.array(self.pose_data[800]).reshape(1, 45).astype(np.float32)
        output = self.engine.inference(pose)
        self.validate_output(output)
...
class TestTHAORTStandardFP16NoEyebrow(TestTHAORTBase):
    """Test THAORT with standard/fp16 model and eyebrow disabled"""
    ...
    def test_standard_no_eyebrow(self):
        """Test standard model without eyebrow has correct decoded_cut_name"""
        self.assertFalse(self.engine.seperable)
        self.assertFalse(self.engine.use_eyebrow)
    
    def test_inference_standard_no_eyebrow(self):
        """Test inference with standard model without eyebrow"""
        self.engine.update_image(self.test_image)
        pose = np.array(self.pose_data[800]).reshape(1, 45).astype(np.float32)
        output = self.engine.inference(pose)
        self.validate_output(output)
...
class TestTHAORTSeperableFP32(TestTHAORTBase):
    """Test THAORT with seperable/fp32 model"""
...
class TestTHAORTEdgeCases(TestTHAORTBase):
    ...
    def test_update_image_wrong_dimensions(self):
        ...
        self.assertIn('Not valid update image', str(context.exception))
...
class TestTHAORTNonDefault(TestTHAORTBase):
    """Test THAORTNonDefault class for non-default device usage"""
    ...
    def setUpClass(cls):
        ...
        cls.engine = THA3ORTNonDefaultSessions(cls.MODEL_DIR, device_id=0, use_eyebrow=True)
    
    def test_nondefault_init(self):
        """Test THAORTNonDefault initialization"""
        self.assertEqual(self.engine.dtype, np.float16)
        self.assertTrue(self.engine.seperable)
        self.assertTrue(self.engine.use_eyebrow)
    ...
    def test_nondefault_update_image(self):
        ...
        self.engine.update_image(self.test_image)
        self.assertIsNotNone(self.engine.decomposed)
        self.assertIsNotNone(self.engine.combined)
...
class TestTHAORTNonDefaultNoEyebrow(TestTHAORTBase):
    """Test THAORTNonDefault class without eyebrow"""
    ...
    def setUpClass(cls):
        ...
        if not os.path.exists(cls.MODEL_DIR):
            raise unittest.SkipTest(f"Model directory not found: {cls.MODEL_DIR}")
        cls.engine = THA3ORTNonDefaultSessions(cls.MODEL_DIR, device_id=0, use_eyebrow=False)
    
    def test_nondefault_no_eyebrow(self):
        """Test THAORTNonDefault without eyebrow"""
        self.assertFalse(self.engine.use_eyebrow)
    ...
    def test_decoded_cut_name_seperable(self):
        """Test that decoded_cut_name is set correctly for seperable model"""
        expected_name = 'morpher_/face_morpher/body/downsample_blocks.3/downsample_blocks.3.3/Relu_output_0'
        self.assertEqual(self.engine.decoded_cut_name, expected_name)


class TestTHAORTNonDefaultStandard(TestTHAORTBase):
    """Test THAORTNonDefault with standard (non-seperable) model"""
    ...
    def test_decoded_cut_name_standard(self):
        """Test that decoded_cut_name is set correctly for standard model"""
        expected_name = 'morpher_/face_morpher/downsample_blocks.3/downsample_blocks.3.2/Relu_output_0'
        self.assertEqual(self.engine.decoded_cut_name, expected_name)
    
    def test_standard_model_inference(self):
        """Test inference with standard model on non-default device"""
        self.engine.update_image(self.test_image)
        pose = np.array(self.pose_data[800]).reshape(1, 45).astype(np.float32)
        output = self.engine.inference(pose)
        self.validate_output(output)
...
class TestTHAORTOutputConsistency(TestTHAORTBase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.model_dirs = {
            'seperable_fp16': './data/tha3/seperable/fp16',
            'standard_fp16': './data/tha3/standard/fp16',
            'standard_fp32': './data/tha3/standard/fp32'
        }
    
    def test_deterministic_output(self):
        """Test that same input produces consistent output"""
        model_dir = self.model_dirs['standard_fp32']
        if not os.path.exists(model_dir):
            self.skipTest(f"Model directory not found: {model_dir}")
        
        engine = THA3ORTSessions(model_dir, use_eyebrow=True)
    ...
    def test_different_poses_different_output(self):
        ...
        model_dir = self.model_dirs['seperable_fp16']
        if not os.path.exists(model_dir):
            self.skipTest(f"Model directory not found: {model_dir}")
...
class TestTHAORTPoseSlicing(TestTHAORTBase):
    ...
    def test_eyebrow_pose_slicing(self):
        ...
        engine = THA3ORTSessions(self.MODEL_DIR, use_eyebrow=True)
        engine.update_image(self.test_image)
...
def THAORT_ShowVideo_SeperableFP16_WithEyebrow():
    """Generate a test video using THAORT with seperable/fp16 model and eyebrow enabled"""
    model_dir = './data/tha3/seperable/fp16'
    if not os.path.exists(model_dir):
    ...
    frames = []
    print("Generating frames with THAORT (seperable/fp16, eyebrow=True):")
    ...
    generate_video(frames, './test/data/tha_ort_seperable_fp16_eyebrow.mp4', 20)


def THAORT_ShowVideo_SeperableFP16_NoEyebrow():
    """Generate a test video using THAORT with seperable/fp16 model and eyebrow disabled"""
    model_dir = './data/tha3/seperable/fp16'
    if not os.path.exists(model_dir):
        print(f"Skipping: Model directory not found: {model_dir}")
        return
    
    engine = THA3ORTSessions(model_dir, use_eyebrow=False)
    ...
    frames = []
    print("Generating frames with THAORT (seperable/fp16, eyebrow=False):")
    from tqdm import tqdm
    for i in tqdm(range(len(pose_data[800:1200]))):
        pose = np.array(pose_data[800 + i]).reshape(1, 45).astype(np.float32)
        output = engine.inference(pose)
    ...
    generate_video(frames, './test/data/tha_ort_seperable_fp16_no_eyebrow.mp4', 20)


def THAORT_ShowVideo_StandardFP16_WithEyebrow():
    """Generate a test video using THAORT with standard/fp16 model and eyebrow enabled"""
    model_dir = './data/tha3/standard/fp16'
    if not os.path.exists(model_dir):
        print(f"Skipping: Model directory not found: {model_dir}")
        return
    ...
    frames = []
    print("Generating frames with THAORT (standard/fp16, eyebrow=True):")
    from tqdm import tqdm
    for i in tqdm(range(len(pose_data[800:1200]))):
        pose = np.array(pose_data[800 + i]).reshape(1, 45).astype(np.float32)
        output = engine.inference(pose)
    ...
    generate_video(frames, './test/data/tha_ort_standard_fp16_eyebrow.mp4', 20)


def THAORT_ShowVideo_SeperableFP32_WithEyebrow():
    """Generate a test video using THAORT with seperable/fp32 model"""
    model_dir = './data/tha3/seperable/fp32'
    if not os.path.exists(model_dir):
        print(f"Skipping: Model directory not found: {model_dir}")
        return
    ...
    frames = []
    print("Generating frames with THAORT (seperable/fp32, eyebrow=True):")
    from tqdm import tqdm
    for i in tqdm(range(len(pose_data[800:1200]))):
        pose = np.array(pose_data[800 + i]).reshape(1, 45).astype(np.float32)
        output = engine.inference(pose)
    ...
    generate_video(frames, './test/data/tha_ort_seperable_fp32_eyebrow.mp4', 20)


def THAORTNonDefault_ShowVideo_SeperableFP16_WithEyebrow():
    """Generate a test video using THAORTNonDefault with seperable/fp16 model"""
    model_dir = './data/tha3/seperable/fp16'
    if not os.path.exists(model_dir):
        print(f"Skipping: Model directory not found: {model_dir}")
        return
    
    engine = THA3ORTNonDefaultSessions(model_dir, device_id=0, use_eyebrow=True)
    ...
    frames = []
    print("Generating frames with THAORTNonDefault (seperable/fp16, eyebrow=True):")
    from tqdm import tqdm
    for i in tqdm(range(len(pose_data[800:1200]))):
        pose = np.array(pose_data[800 + i]).reshape(1, 45).astype(np.float32)
        output = engine.inference(pose)
    ...
    generate_video(frames, './test/data/tha_ort_nondefault_seperable_fp16_eyebrow.mp4', 20)


def THAORTNonDefault_ShowVideo_SeperableFP16_NoEyebrow():
    """Generate a test video using THAORTNonDefault with seperable/fp16 model, no eyebrow"""
    model_dir = './data/tha3/seperable/fp16'
    if not os.path.exists(model_dir):
        print(f"Skipping: Model directory not found: {model_dir}")
        return
    
    engine = THA3ORTNonDefaultSessions(model_dir, device_id=0, use_eyebrow=False)
    ...
    frames = []
    print("Generating frames with THAORTNonDefault (seperable/fp16, eyebrow=False):")
    from tqdm import tqdm
    for i in tqdm(range(len(pose_data[800:1200]))):
        pose = np.array(pose_data[800 + i]).reshape(1, 45).astype(np.float32)
        output = engine.inference(pose)
    ...
    generate_video(frames, './test/data/tha_ort_nondefault_seperable_fp16_no_eyebrow.mp4', 20)


def THAORTNonDefault_ShowVideo_StandardFP16_WithEyebrow():
    """Generate a test video using THAORTNonDefault with standard/fp16 model"""
    model_dir = './data/tha3/standard/fp16'
    if not os.path.exists(model_dir):
        print(f"Skipping: Model directory not found: {model_dir}")
        return
    ...
    frames = []
    print("Generating frames with THAORTNonDefault (standard/fp16, eyebrow=True):")
    from tqdm import tqdm
    for i in tqdm(range(len(pose_data[800:1200]))):
        pose = np.array(pose_data[800 + i]).reshape(1, 45).astype(np.float32)
        output = engine.inference(pose)
    ...
    generate_video(frames, './test/data/tha_ort_nondefault_standard_fp16_eyebrow.mp4', 20)


def THAORTNonDefault_ShowVideo_StandardFP16_NoEyebrow():
    """Generate a test video using THAORTNonDefault with standard/fp16 model, no eyebrow"""
    model_dir = './data/tha3/standard/fp16'
    if not os.path.exists(model_dir):
        print(f"Skipping: Model directory not found: {model_dir}")
        return
    ...
    frames = []
    print("Generating frames with THAORTNonDefault (standard/fp16, eyebrow=False):")
    from tqdm import tqdm
    for i in tqdm(range(len(pose_data[800:1200]))):
        pose = np.array(pose_data[800 + i]).reshape(1, 45).astype(np.float32)
        output = engine.inference(pose)
    ...
    generate_video(frames, './test/data/tha_ort_nondefault_standard_fp16_no_eyebrow.mp4', 20)


# ============================================================================
# Additional THAORTNonDefault Tests
# ============================================================================

class TestTHAORTNonDefaultFP32(TestTHAORTBase):
    """Test THAORTNonDefault with FP32 models"""
    ...
    def test_seperable_detection(self):
        """Test seperable detection for FP32 model"""
        self.assertTrue(self.engine.seperable)
...
class TestTHAORTNonDefaultMultipleImages(TestTHAORTBase):
    """Test THAORTNonDefault with multiple image updates"""
    
    MODEL_DIR = './data/tha3/standard/fp32'
...
class TestTHAORTNonDefaultEdgeCases(TestTHAORTBase):
    """Test edge cases for THAORTNonDefault"""
    ...
    def setUpClass(cls):
        ...
        if not os.path.exists(cls.MODEL_DIR):
            raise unittest.SkipTest(f"Model directory not found: {cls.MODEL_DIR}")
        cls.engine_eyebrow = THA3ORTNonDefaultSessions(cls.MODEL_DIR, device_id=0, use_eyebrow=True)
        cls.engine_no_eyebrow = THA3ORTNonDefaultSessions(cls.MODEL_DIR, device_id=0, use_eyebrow=False)
    
    def test_extreme_pose_values_nondefault(self):
        """Test extreme pose values with THAORTNonDefault"""
    ...
    def test_negative_pose_values_nondefault(self):
        """Test negative pose values with THAORTNonDefault"""
        self.engine_eyebrow.update_image(self.test_image)
        negative_pose = np.ones((1, 45), dtype=np.float32) * -50.0
    ...
    def test_rapid_inferences_nondefault(self):
        """Test rapid consecutive inferences with THAORTNonDefault"""
        self.engine_eyebrow.update_image(self.test_image)
        for i in range(20):
            pose = np.array(self.pose_data[800 + i]).reshape(1, 45).astype(np.float32)
    ...
    def test_eyebrow_vs_no_eyebrow_output_difference(self):
        ...
        # With different eyebrow handling, outputs may differ
        # This test just ensures both work correctly
        self.validate_output(output_eyebrow)
        self.validate_output(output_no_eyebrow)


class TestTHAORTNonDefaultAllConfigurations(TestTHAORTBase):
    """Test THAORTNonDefault across all available model configurations"""
    
    MODEL_CONFIGS = [
        ('./data/tha3/seperable/fp16', True, True),    # seperable, fp16, with eyebrow
        ('./data/tha3/seperable/fp16', True, False),   # seperable, fp16, without eyebrow
        ('./data/tha3/seperable/fp32', True, True),    # seperable, fp32, with eyebrow
        ('./data/tha3/seperable/fp32', True, False),   # seperable, fp32, without eyebrow
        ('./data/tha3/standard/fp16', False, True),    # standard, fp16, with eyebrow
        ('./data/tha3/standard/fp16', False, False),   # standard, fp16, without eyebrow
        ('./data/tha3/standard/fp32', False, True),    # standard, fp32, with eyebrow
        ('./data/tha3/standard/fp32', False, False),   # standard, fp32, without eyebrow
    ]
    
    def test_all_configurations(self):
        """Test THAORTNonDefault with all available configurations"""
        for model_dir, is_seperable, use_eyebrow in self.MODEL_CONFIGS:
            if not os.path.exists(model_dir):
                continue
            
            with self.subTest(model_dir=model_dir, is_seperable=is_seperable, use_eyebrow=use_eyebrow):
                engine = THA3ORTNonDefaultSessions(model_dir, device_id=0, use_eyebrow=use_eyebrow)
                
                # Verify configuration detection
                self.assertEqual(engine.seperable, is_seperable)
...
class TestTHAORTCompareDefaultAndNonDefault(TestTHAORTBase):
    """Compare outputs between THAORT and THAORTNonDefault"""
    ...
    def setUpClass(cls):
        ...
        if not os.path.exists(cls.MODEL_DIR):
            raise unittest.SkipTest(f"Model directory not found: {cls.MODEL_DIR}")
        cls.engine_default = THA3ORTSessions(cls.MODEL_DIR, use_eyebrow=True)
        cls.engine_nondefault = THA3ORTNonDefaultSessions(cls.MODEL_DIR, device_id=0, use_eyebrow=True)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == '--show-all':
            ...
            print("=" * 60)
            print("Running all THAORT video showcase tests")
            print("=" * 60)
            THAORT_ShowVideo_SeperableFP16_WithEyebrow()
            THAORT_ShowVideo_SeperableFP16_NoEyebrow()
            THAORT_ShowVideo_StandardFP16_WithEyebrow()
            THAORT_ShowVideo_SeperableFP32_WithEyebrow()
            print("\n" + "=" * 60)
            print("Running all THAORTNonDefault video showcase tests")
            print("=" * 60)
            THAORTNonDefault_ShowVideo_SeperableFP16_WithEyebrow()
            THAORTNonDefault_ShowVideo_SeperableFP16_NoEyebrow()
            THAORTNonDefault_ShowVideo_StandardFP16_WithEyebrow()
            THAORTNonDefault_ShowVideo_StandardFP16_NoEyebrow()
        elif sys.argv[1] == '--show-thaort':
            print("=" * 60)
            print("Running THAORT video showcase tests")
            print("=" * 60)
            THAORT_ShowVideo_SeperableFP16_WithEyebrow()
            THAORT_ShowVideo_SeperableFP16_NoEyebrow()
            THAORT_ShowVideo_StandardFP16_WithEyebrow()
            ...
            print("=" * 60)
            print("Running THAORTNonDefault video showcase tests")
            print("=" * 60)
            THAORTNonDefault_ShowVideo_SeperableFP16_WithEyebrow()
            THAORTNonDefault_ShowVideo_SeperableFP16_NoEyebrow()
            THAORTNonDefault_ShowVideo_StandardFP16_WithEyebrow()
